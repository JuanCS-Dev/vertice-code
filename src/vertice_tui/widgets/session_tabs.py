...
class SessionTabs(Widget):
    ...
    def on_tabbed_content_tab_activated(self, event: TabbedContent.TabActivated) -> None:
        """Handle tab activation with enhanced visual persistence."""
        pane = event.pane
        if isinstance(pane, SessionPane):
            # Save current session state before switching
            if self.active_session_id:
                self._save_session_visual_state(self.active_session_id)

            # Switch to new session
            self.active_session_id = pane.session.id

            # Restore visual state for new session
            self._restore_session_visual_state(pane.session.id)
    ...
    def _save_session_visual_state(self, session_id: str) -> None:
        """Save visual state of a session for persistence."""
        if session_id not in self._sessions:
            return
        ...
        try:
            # Find the pane content widget
            pane = self._tabbed_content.get_pane(f"session-{session_id}")
            if pane:
                # Save scroll position (if available)
                content_widget = pane.query_one(f"#content-{session_id}")
                if hasattr(content_widget, "scroll_y"):
                    session.scroll_position = int(content_widget.scroll_y)

                # Save viewport content (snapshot of visible content)
                if hasattr(content_widget, "renderable"):
                    # This is a simplified approach - in practice you'd capture
    ...
    def _restore_session_visual_state(self, session_id: str) -> None:
        """Restore visual state of a session."""
        if session_id not in self._sessions:
            return
        ...
        try:
            # Find the pane content widget
            pane = self._tabbed_content.get_pane(f"session-{session_id}")
            if pane and session.scroll_position > 0:
                content_widget = pane.query_one(f"#content-{session_id}")
                if hasattr(content_widget, "scroll_y"):
