...
class ResilienceMixin:
    ...
    def _get_circuit(self, provider: str) -> CircuitBreaker:
        ...
        if provider not in self._circuits:
            self._circuits[provider] = CircuitBreaker(name=provider, config=self.CIRCUIT_CONFIG)
    ...
    def get_resilience_stats(self) -> Dict[str, Any]:
        ...
        # Add circuit stats
        stats["circuits"] = {name: circuit.get_stats() for name, circuit in self._circuits.items()}
