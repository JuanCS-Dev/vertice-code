...
class ConfidenceRouter:
    ...
    def __init__(self, config: Optional[AutonomyConfig] = None):
        ...
        self._high_risk_patterns = [re.compile(p, re.IGNORECASE) for p in self.HIGH_RISK_PATTERNS]
    ...
    def route(
        ...
        if (
            cal_confidence >= self._config.high_confidence_threshold
            and decision.risk_level < self._config.high_risk_threshold
        ):
        ...
        # Low confidence or high risk = require approval
            ...
            decision.autonomy_level = AutonomyLevel.APPROVE
            decision.can_proceed = False
            decision.reasoning = f"Low confidence ({cal_confidence:.2f}), approval required"
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        avg_confidence = (
            sum(d.confidence.calibrated_confidence for d in self._routing_history) / total
        )
