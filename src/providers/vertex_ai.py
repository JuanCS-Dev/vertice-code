...
class VertexAIProvider:
    ...
    MODELS = {
        # Default Aliases -> Map to Gemini 3 (User Mandate)
        "flash": "gemini-3-flash-preview",
        "pro": "gemini-3-pro-preview",
        "gemini-3-pro": "gemini-3-pro-preview",
        "gemini-3-flash": "gemini-3-flash-preview",
        # Legacy / Fallback mapping (Forced Upgrade)
    ...
    def _init_legacy_model(self):
        ...
        if not self._legacy_model and HAS_LEGACY_SDK:
            try:
                ...
                logger.info(
                    f"✅ Legacy Vertex Model initialized: {self.model_id} @ {self.location}"
                )
            except Exception as e:
    ...
    async def generate(
        self, prompt: str, system_instruction: Optional[str] = None, stream: bool = False, **kwargs
        ...
        async for chunk in self.stream_chat(
            messages=messages, system_prompt=system_instruction, **kwargs
        ):
            yield chunk
    ...
    async def _stream_chat_v3(self, messages, system_prompt, max_tokens, temperature, tools):
    ...
    async def _stream_chat_v2(self, messages, system_prompt, max_tokens, temperature, tools):
        ...
        try:
            ...
            model = GenerativeModel(
                self.model_id, system_instruction=system_prompt, tools=vertex_tools
            )
            ...
            responses = await chat.send_message_async(
                last_msg, generation_config=config, stream=True
            )
