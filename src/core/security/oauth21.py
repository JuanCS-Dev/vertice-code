...
class ProtectedResourceMetadata:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ProtectedResourceMetadata":
        ...
        return cls(
            ...
            scopes_supported=data.get("scopes_supported", []),
            bearer_methods_supported=data.get(
...
class OAuth21Client:
    ...
    async def discover(self, resource_url: str) -> None:
        ...
        if not self._resource_meta.authorization_servers:
            raise ValueError(
                f"No authorization servers found for resource: {resource_url}"
            )
    ...
    async def _discover_resource_metadata(
        ...
        logger.warning(
            f"No Protected Resource Metadata found for {resource_url}, "
    ...
    def create_authorization_url(
        self, extra_scopes: Optional[List[str]] = None
    ) -> tuple[str, str]:
    ...
    async def exchange_code(self, code: str) -> TokenResponse:
        ...
        logger.info(
            f"Token obtained: expires_in={self._token.expires_in}s, "
            f"scope={self._token.scope}"
        )
        return self._token
