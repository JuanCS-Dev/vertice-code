...
class ConsentRequest:
    ...
    def to_display_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "consent_level": DEFAULT_CONSENT_LEVELS.get(
                self.category, ConsentLevel.CONFIRM
            ).value,
        }
...
class ConsentManager:
    ...
    def set_consent_level(
        self, category: OperationCategory, level: ConsentLevel
    ) -> None:
    ...
    async def request_consent(
        ...
        if level == ConsentLevel.NOTIFY:
            logger.info(
                f"Consent notification: {tool_name} - {operation}"
            )
        ...
        if not self._consent_callback:
            logger.warning(
                f"No consent callback set, denying: {tool_name} - {operation}"
            )
            ...
            if consent_manager:
                ...
                if not granted:
                    raise PermissionError(
                        f"Consent denied for {func.__name__}: {operation}"
                    )
