...
class TestChatE2EVertex:
    ...
    async def test_simple_hello_stream(self, dev_auth_headers):
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            assert (
                len(finish_lines) == 1
            ), f"Expected 1 finish signal, got {len(finish_lines)}: {content}"

            # Verify finish reason
            assert (
                '"finishReason"' in finish_lines[0]
            ), f"Missing finishReason in: {finish_lines[0]}"
    ...
    async def test_stream_code_generation(self, dev_auth_headers):
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            assert (
                "def " in full_text or "fibonacci" in full_text.lower()
            ...
            response = await client.post(
                ...
                json={
                    "messages": [
                        ...
                        {
                            "role": "assistant",
                            "content": "Got it! I'll remember that the secret word is BUTTERFLY.",
                        },
                        {
                            "role": "user",
