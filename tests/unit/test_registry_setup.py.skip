...
class TestSetupDefaultTools:
    ...
    def test_default_setup_registers_file_ops_tools(self):
        """Default setup should register file operation tools."""
        registry, mcp = setup_default_tools()

        # Check that at least some tools are registered
        tool_count = len(registry.tools)
        assert tool_count > 0, f"Registry should have tools, got {tool_count}"

        # Check that registry has some expected categories
        if hasattr(registry, "get_categories"):
            categories = registry.get_categories()
            assert len(categories) > 0, "Registry should have categories"
...
class TestEdgeCases:
    ...
    def test_duplicate_tool_registration_handled_gracefully(self):
        ...
        registry, mcp = setup_default_tools(custom_tools=[mock_tool, mock_tool])
    ...
    def test_registry_with_no_tools_is_valid(self):
        ...
        registry, mcp = setup_default_tools(
            include_file_ops=False, include_bash=False, include_search=False, include_git=False
        )
