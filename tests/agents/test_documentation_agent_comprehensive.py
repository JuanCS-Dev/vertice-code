...
class TestDocumentationGeneration:
    ...
    def test_generate_readme_basic(self, agent, tmp_path):
        ...
        result = agent.generate_readme(
            project_name="TestProject", description="A test project", output_path=str(output)
        )
    ...
    def test_readme_has_title(self, agent, tmp_path):
        output = tmp_path / "README.md"
        agent.generate_readme(project_name="MyProject", output_path=str(output))
        content = output.read_text()
        assert "# MyProject" in content

    def test_readme_has_sections(self, agent, tmp_path):
        output = tmp_path / "README.md"
        agent.generate_readme(project_name="Test", output_path=str(output))
        content = output.read_text()
    ...
    def test_readme_with_badges(self, agent, tmp_path):
        output = tmp_path / "README.md"
        agent.generate_readme(project_name="Test", add_badges=True, output_path=str(output))
        content = output.read_text()
...
"""
        ...
        result = agent.generate_api_docs(source_dir=str(tmp_path), output_path=str(output))
        assert result["success"] is True
        assert output.exists()
...
"""
    ...
    def test_changelog_formatting(self, agent, tmp_path):
        ...
        agent.generate_changelog(
            version="2.0.0", changes=["Breaking: Changed API"], output_path=str(output)
        )
        content = output.read_text()
    ...
    def test_generate_license(self, agent, tmp_path):
        ...
        result = agent.generate_license(
            license_type="MIT", author="Test Author", year=2025, output_path=str(output)
        )
        assert result["success"] is True
        assert output.exists()
    ...
    def test_generate_mkdocs_config(self, agent, tmp_path):
        ...
        result = agent.generate_mkdocs_config(project_name="Test", output_path=str(output))
        assert result["success"] is True
        assert output.exists()

    def test_generate_sphinx_config(self, agent, tmp_path):
        output = tmp_path / "conf.py"
        result = agent.generate_sphinx_config(project_name="Test", output_path=str(output))
        assert result["success"] is True
...
'''
    ...
    def test_generate_tutorial(self, agent, tmp_path):
        ...
        result = agent.generate_tutorial(
            topic="Getting Started", content=["Step 1", "Step 2"], output_path=str(output)
        )
        assert result["success"] is True

    def test_generate_faq(self, agent, tmp_path):
        output = tmp_path / "FAQ.md"
        faqs = [{"question": "How to install?", "answer": "Run pip install"}]
    ...
    def test_generate_architecture_doc(self, agent, tmp_path):
        ...
        result = agent.generate_architecture_doc(project_dir=str(tmp_path), output_path=str(output))
        assert result["success"] is True
