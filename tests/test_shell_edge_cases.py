...
class TestShellEdgeCases:
    ...
    def test_special_chars_in_args(self):
        result = subprocess.run(
            ["echo", 'hello "world"', "$HOME"], capture_output=True, text=True, timeout=5
        )
        assert result.returncode == 0
    ...
    def test_long_command_line(self):
        long_arg = "x" * 10000
        result = subprocess.run(["echo", long_arg], capture_output=True, text=True, timeout=5)
        assert result.returncode == 0
    ...
    def test_nonexistent_command(self):
        with pytest.raises((subprocess.CalledProcessError, FileNotFoundError, OSError)):
            subprocess.run(
                ["nonexistent_xyz123"], capture_output=True, text=True, timeout=2, check=True
            )
    ...
    def test_permission_denied(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with pytest.raises(
                (subprocess.CalledProcessError, PermissionError, FileNotFoundError, OSError)
            ):
...
class TestRealWorldUsage:
    def test_git_workflow(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                subprocess.run(
                    ["git", "config", "user.email", "test@test.com"], timeout=5, check=True
                )
                ...
                result = subprocess.run(
                    ["git", "commit", "-m", "Initial"], capture_output=True, text=True, timeout=5
                )
                assert result.returncode == 0
        finally:
    ...
    def test_python_script(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                ["python3", str(script)], capture_output=True, text=True, timeout=5
            )
            assert result.returncode == 0
    ...
    def test_file_operations(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                ["grep", "-r", "world", tmpdir], capture_output=True, text=True, timeout=5
            )
    ...
    def test_env_vars(self):
        ...
        result = subprocess.run(
            ["bash", "-c", "echo $TEST_VAR"], env=env, capture_output=True, text=True, timeout=5
        )
...
class TestUnicode:
    def test_unicode_io(self):
        ...
        result = subprocess.run(["echo", unicode_text], capture_output=True, text=True, timeout=5)
        assert result.returncode == 0
