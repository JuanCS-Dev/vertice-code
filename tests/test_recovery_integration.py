...
TOOL_CALL: {"tool": "test_tool", "args": {"corrected": true}}"""
    ...
    def test_initialization_without_enhancements(self, mock_llm):
        ...
        engine = ErrorRecoveryEngine(
            llm_client=mock_llm, enable_retry_policy=False, enable_circuit_breaker=False
        )
    ...
    async def test_recovery_with_backoff_success(self, mock_llm, recovery_context):
        ...
        result = await engine.attempt_recovery_with_backoff(
            recovery_context, Exception("Test error")
        )
    ...
    @pytest.mark.asyncio
    async def test_recovery_with_backoff_applies_delay(
        self, mock_sleep, mock_llm, recovery_context
    ):
        ...
        await engine.attempt_recovery_with_backoff(
            recovery_context, Exception("Transient error: timeout")
        )
    ...
    async def test_circuit_breaker_prevents_recovery(self, mock_llm, recovery_context):
        ...
        engine = ErrorRecoveryEngine(
            llm_client=mock_llm, enable_retry_policy=False, enable_circuit_breaker=True
        )
    ...
    async def test_retry_policy_rejects_permanent_errors(self, mock_llm, recovery_context):
        ...
        result = await engine.attempt_recovery_with_backoff(recovery_context, permanent_error)
    ...
    async def test_retry_policy_accepts_transient_errors(self, mock_llm, recovery_context):
        ...
        result = await engine.attempt_recovery_with_backoff(recovery_context, transient_error)
    ...
    def test_get_circuit_breaker_status(self, mock_llm):
        ...
        engine = ErrorRecoveryEngine(llm_client=mock_llm, enable_circuit_breaker=True)
    ...
    def test_get_circuit_breaker_status_disabled(self, mock_llm):
        ...
        engine = ErrorRecoveryEngine(llm_client=mock_llm, enable_circuit_breaker=False)
    ...
    async def test_recovery_updates_circuit_breaker_on_failure(self, mock_llm, recovery_context):
        ...
        await engine.attempt_recovery_with_backoff(recovery_context, Exception("Test error"))
