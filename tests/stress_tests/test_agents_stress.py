...
async def test_agent_manager_concurrency(agent_manager):
    ...
    for i in range(10):
        task = AgentTask(request=f"Stress test request {i}", context={"file": f"test_{i}.py"})
...
async def test_explorer_deep_search_performance():
    ...
    task = AgentTask(request="deep search for 'BaseAgent'", context={"cwd": str(project_root)})
...
async def test_refactorer_blast_radius_integration():
    ...
    explorer_mock.execute.return_value.data = {"relevant_files": [{"path": "dependent_module.py"}]}
...
async def test_devops_structured_output():
    ...
    task = AgentTask(request="deploy to k8s", context={"env": "prod"})
...
async def test_documentation_fallback_resilience():
    ...
    with tempfile.NamedTemporaryFile(suffix=".py", mode="w+") as tmp:
        ...
        task = AgentTask(request="generate docs", context={"files": [tmp.name]})
