...
async def test_router_selects_prometheus_for_complex_tasks():
    ...
    # Test COMPLEX
    task_complex = Task(
        id="t1", description="Build a full autonomous system", complexity=TaskComplexity.COMPLEX
    )
...
async def test_orchestrator_delegates_to_prometheus():
    ...
    mock_result = TaskResult(
        task_id="t1", status=TaskStatus.COMPLETED, output={"output": "Prometheus Plan Executed"}
    )
    mock_response = AgentResponse(
        success=True, data={"task_id": "t1", "result": mock_result}, reasoning="Simulated"
    )
    ...
    with patch("vertice_agents.registry.get_agent", return_value=mock_prometheus):
        ...
        task = Task(id="t1", description="Complex task", complexity=TaskComplexity.COMPLEX)

        chunks = []
        async for chunk in orchestrator._delegate_to_agent(
            mock_prometheus, AgentRole.PROMETHEUS, task, stream=True
        ):
            chunks.append(chunk)
