...
class QualityValidator:
    ...
    def validate_code_output(
        self, request: str, output: str, expected_elements: List[str]
    ) -> QualityScore:
    ...
    def validate_explanation_output(
        self, request: str, output: str, key_concepts: List[str]
    ) -> QualityScore:
    ...
    def validate_plan_output(
        self, request: str, output: str, expected_steps: List[str]
    ) -> QualityScore:
    ...
    def _check_coherence(self, output: str) -> float:
        ...
        if any(
            ...
            for word in ["in summary", "to summarize", "in conclusion", "finally"]
        ):
            score += 0.1
    ...
    def _check_plan_actionability(self, output: str) -> float:
        ...
        if any(
            tech in output.lower()
            for tech in ["python", "javascript", "docker", "git", "api", "database"]
        ):
            score += 0.2
...
class QualityTester:
    ...
    async def execute_and_validate(
        self, request: str, validation_type: str, expected_elements: List[str], timeout: int = 120
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if validation_type == "code":
                score = self.validator.validate_code_output(request, output, expected_elements)
                ...
                score = self.validator.validate_plan_output(request, output, expected_elements)
            else:
                score = self.validator.validate_explanation_output(
                    request, output, expected_elements
                )
...
class TestCodeGenerationQuality:
    ...
    async def test_function_completeness(self, quality_tester):
        ...
        result = await quality_tester.execute_and_validate(request, "code", expected)
        ...
        # Quality threshold
        assert (
            result["quality_score"].overall >= 0.6
        ), f"Quality too low: {result['quality_score'].overall:.1%}"

        # Must have working code
        assert result["quality_score"].correctness >= 0.5, "Code correctness too low"
    ...
    async def test_class_implementation(self, quality_tester):
        ...
        assert result["quality_score"].completeness >= 0.6, "Should implement all required methods"
    ...
    async def test_algorithm_correctness(self, quality_tester):
        ...
        assert result["quality_score"].correctness >= 0.6, "Algorithm correctness too low"
...
class TestExplanationQuality:
    ...
    async def test_concept_explanation(self, quality_tester):
        ...
        result = await quality_tester.execute_and_validate(request, "explanation", expected)
        ...
        assert result["quality_score"].completeness >= 0.5, "Explanation not complete enough"
    ...
    async def test_comparison_explanation(self, quality_tester):
        ...
        assert result["quality_score"].completeness >= 0.6, "Should compare both REST and GraphQL"
...
class TestPlanningQuality:
    ...
    async def test_project_plan(self, quality_tester):
        ...
        result = await quality_tester.execute_and_validate(request, "plan", expected)
        ...
        assert result["quality_score"].actionability >= 0.5, "Plan should be actionable"
    ...
    async def test_refactoring_plan(self, quality_tester):
        ...
        assert result["quality_score"].coherence >= 0.5, "Plan should be logically structured"
...
class TestComprehensiveQuality:
    ...
    async def test_multi_part_request(self, quality_tester):
        ...
        """
        expected = ["def", "password", "test", "assert", "docstring", "uppercase", "number", "8"]
    ...
    async def test_error_handling_request(self, quality_tester):
        ...
        assert has_error_handling, "Code should include error handling when requested"
    ...
    async def test_quality_benchmark_suite(self, quality_tester):
        ...
        for case in test_cases:
            result = await quality_tester.execute_and_validate(
                case["request"], case["type"], case["expected"]
            )
            results.append(
                {
        ...
        # Benchmark threshold
        assert avg_score >= 0.5, f"Average quality score too low: {avg_score:.1%}"
