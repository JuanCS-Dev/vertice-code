...
def audit_logger(tmp_path):
    ...
    return AuditLogger(log_dir=str(tmp_path / "security_logs"), enable_file_logging=True)
...
class TestCommandInjection:
    ...
    def test_blocks_command_injection(self, input_validator, payload):
        ...
        assert result.threat_level in ["HIGH", "CRITICAL"], f"Should flag as high threat: {payload}"
    ...
    def test_logs_injection_attempt(self, input_validator, audit_logger):
        ...
        input_validator.validate_command(f"ls {payload}")
...
class TestPathTraversal:
    ...
    def test_blocks_path_traversal(self, input_validator, path):
        ...
        assert result.threat_level in ["HIGH", "CRITICAL"], f"Should flag as high threat: {path}"
...
class TestPromptInjection:
    ...
    def test_blocks_prompt_injection(self, prompt_shield, injection):
        ...
        assert result.is_suspicious or result.blocked, f"Should detect injection: {injection}"
    ...
    def test_blocks_encoding_bypass(self, prompt_shield):
        ...
        for injection in encoded_injections:
            ...
            assert (
                result.needs_review or result.is_suspicious
...
class TestPythonSandboxEscape:
    ...
    def test_blocks_sandbox_escape(self, python_sandbox, exploit):
        ...
        assert not result.success or result.blocked, f"Should block exploit: {exploit}"
        assert (
            "allowed" not in str(result.output).lower() or result.blocked
...
"""
        ...
        python_sandbox.execute(safe_code)
...
class TestResourceExhaustion:
    ...
    def test_blocks_fork_bomb(self, secure_executor):
        ...
        for bomb in fork_bombs:
            ...
            assert (
                result.blocked or result.timed_out or not result.success
...
"""
        ...
        assert not result.success or result.memory_exceeded, "Should enforce memory limit"
...
"""
        ...
        assert result.timed_out or not result.success, "Should enforce CPU/time limit"
    ...
    def test_enforces_output_limit(self, secure_executor):
        ...
        assert (
            result.output_truncated or len(result.stdout or "") < 10_000_000
...
"""
        ...
        secure_executor.execute(f'python3 -c "{file_bomb}"', timeout=10)
...
class TestPrivilegeEscalation:
    ...
    def test_cannot_access_root_files(self, secure_executor):
        ...
        assert not result.success or "Permission denied" in (
            result.stderr or ""
    ...
    def test_cannot_change_permissions(self, secure_executor, attack_workspace):
        ...
        assert not result.success or result.blocked, "Should not allow permission changes"

    def test_cannot_use_sudo(self, secure_executor):
        """Script kid cannot use sudo."""
        result = secure_executor.execute("sudo id")

        assert not result.success or result.blocked, "Should block sudo"

    def test_environment_sanitized(self, secure_executor):
        """Dangerous environment variables are removed."""
        result = secure_executor.execute("echo $LD_PRELOAD $LD_LIBRARY_PATH", inherit_env=False)

        output = result.stdout or ""
        assert (
            "LD_PRELOAD" not in output or output.strip() == ""
...
class TestSecurityAudit:
    ...
    def test_rate_limits_repeated_attacks(self, input_validator):
        ...
        input_validator.validate_command("ls")
