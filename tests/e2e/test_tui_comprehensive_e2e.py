...
class TUIComprehensiveE2E:
        ...
        try:
            ...
            if metrics.latency_total_ms > 0:
                metrics.throughput_tokens_per_sec = (
                    metrics.tokens_generated / metrics.latency_total_ms
                ) * 1000
    ...
    async def test_throughput(self) -> E2ETestResult:
        ...
        try:
            ...
            metrics.tokens_generated = len(output.split())
            metrics.throughput_tokens_per_sec = (
                (metrics.tokens_generated / metrics.latency_total_ms) * 1000
                if metrics.latency_total_ms > 0
                else 0
            )
    ...
    async def test_coder_agent_orchestration(self) -> E2ETestResult:
        ...
        try:
            ...
            passed = metrics.chunks_received > 0 and (
                "def " in output or "function" in output.lower()
            )
...
The function currently reads a file synchronously. Provide numbered steps."""
    ...
    async def run_all(self) -> Dict[str, Any]:
        ...
        for category, test_func in tests:
            ...
            if result.quality:
                print(
                    f"  └─ Quality: {result.quality.score}/{result.quality.max_score} ({result.quality.grade.value})"
                )
            if result.error:
...
async def test_code_quality(e2e_suite):
    ...
    assert (
        result.passed
    ), f"Code quality test failed: score={result.quality.score if result.quality else 0}"
...
async def test_plan_quality(e2e_suite):
    ...
    assert (
        result.passed
    ), f"Plan quality test failed: score={result.quality.score if result.quality else 0}"
