...
async def run_quality_test():
    ...
    parser.add_argument(
        "--model", type=str, default="gemini-2.0-flash-exp", help="Model name for real execution"
    )
    parser.add_argument(
        "--scenario", type=str, default=None, help="Scenario name (e.g. complex_feature)"
    )
    ...
    if args.scenario:
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "scenario", f"scripts/e2e/scenarios/{args.scenario}.py"
            )
    ...
    if args.real:
        ...
        if args.scenario:
            print(
                "⚠️ Mock mode only supports default Fibonacci scenario fully. Running with generic mock response..."
            )
            script = [
                "I am a mock response. I cannot execute dynamic scenarios without a predefined script."
            ]
        else:
            # Default Fib Script
        ...
        controller.agents.router = type(
            "MockRouter",
            (),
            {"route": lambda self, m: None, "get_routing_suggestion": lambda self, m: None},
        )()
