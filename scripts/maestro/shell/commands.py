...
(schema, query, sql, migration, table, index, etc.)[/dim]""",
        ...
        if c.startswith("/commands"):
            ...
            for idx, cmd in enumerate(suggestions, 1):
                table.add_row(str(idx), cmd["command"], cmd["description"], cmd["category"])
        ...
        if c == "/permissions":
            ...
            try:
                ...
                status_table.add_row(
                    "Mode",
                    "[green]Safe Mode Enabled[/green]"
                    if pm.safe_mode
                    else "[red]Safe Mode Disabled[/red]",
                )
                status_table.add_row(
                    "Allow Rules", f"[cyan]{summary['allow_count']}[/cyan] patterns"
                )
                status_table.add_row("Deny Rules", f"[red]{summary['deny_count']}[/red] patterns")
                status_table.add_row(
                    "Auto-approve Read-only",
                    "[green]Yes[/green]" if summary["auto_approve_enabled"] else "[dim]No[/dim]",
                )
                status_table.add_row(
                    "Audit Logging",
                    "[green]Enabled[/green]" if summary["audit_enabled"] else "[dim]Disabled[/dim]",
                )
                ...
                # Allow list table
                allow_table = Table(
                    title="✅ Allow List (Auto-approved)", border_style="green", show_lines=True
                )
                ...
                if len(pm.config["permissions"]["allow"]) > 20:
                    allow_table.add_row(
                        f"[dim]... and {len(pm.config['permissions']['allow']) - 20} more[/dim]"
                    )
                ...
                # Deny list table
                deny_table = Table(
                    title="🛑 Deny List (Blocked)", border_style="red", show_lines=True
                )
                ...
                if len(pm.config["permissions"]["deny"]) > 20:
                    deny_table.add_row(
                        f"[dim]... and {len(pm.config['permissions']['deny']) - 20} more[/dim]"
                    )
                ...
                # Help text
                self.c.print(
                    "[dim]Tip: Use [bold]'always'[/bold] when approving commands to add them to the allow list[/dim]\n"
                )
        ...
        if c == "/metrics":
            ...
            try:
                ...
                exec_table.add_row("Total Executions", str(metrics.get("executions", 0)), "")
                exec_table.add_row("Success Rate", f"{success_rate:.2f}%", status_emoji)
                exec_table.add_row(
                    "Average Latency",
                    f"{metrics.get('avg_latency', 0):.3f}s",
                    "⚡" if metrics.get("avg_latency", 0) < 0.5 else "🐌",
                )
                exec_table.add_row("Total Runtime", f"{metrics.get('total_time', 0):.2f}s", "")
