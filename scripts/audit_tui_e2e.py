...
def test_bridge_initialization():
    ...
    try:
        ...
        if bridge.agents is not None:
            count = len(bridge.agents.available_agents) if hasattr(bridge.agents, 'available_agents') else 0
...
def test_managers(bridge):
    ...
    try:
        ...
        log_result("managers", "MemoryManager.read_memory", True, f"success={memory.get('success', False)}")
    except Exception as e:
    ...
    try:
        ...
        log_result("managers", "ContextManager.get_token_stats", True, f"tokens tracked")
    except Exception as e:
...
def test_tools(bridge):
    ...
    try:
        ...
        critical_tools = ["read_file", "write_file", "search_files", "bash_command", "list_directory"]
    ...
    try:
        ...
        if result.get("success"):
            log_result("tools", "execute: read_file", True, f"{len(str(result.get('data', '')))} chars")
        else:
...
def test_agents(bridge):
    ...
    try:
        ...
        available = bridge.agents.available_agents if hasattr(bridge.agents, 'available_agents') else []
