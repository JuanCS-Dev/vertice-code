...
async def run_agent(name: str, agent, prompt: str) -> Dict:
    ...
    try:
        task = AgentTask(
            ...
            metadata={'interface': 'gather_context_script', 'timestamp': datetime.now().isoformat()}
        )
        ...
        return {"name": name, "success": True, "data": response.data if hasattr(response, 'data') else response}
...
async def main():
    ...
    agents = {
        'Executor': NextGenExecutorAgent(llm, mcp, execution_mode=ExecutionMode.LOCAL, security_level=SecurityLevel.PERMISSIVE, approval_callback=None),
        'Explorer': explorer,
