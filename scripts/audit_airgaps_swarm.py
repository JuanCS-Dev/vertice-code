...
class AuditAgent:
    ...
    def report_issue(self, severity: str, description: str, location: str = "General"):
        self.issues.append({"severity": severity, "description": description, "location": location})
...
class ProviderAudit(AuditAgent):
    async def audit(self):
        ...
        if env_loc == "global":
            self.report_issue(
                "CRITICAL",
                "VERTEX_AI_LOCATION is set to 'global'. This causes immediate 404s/crashes with recent SDKs.",
                ".env / Shell Env",
            )
        ...
        for tool_file in tools_dir.glob("*.py"):
            ...
            if "class" in content and "(Tool)" in content:
                ...
                if "def get_schema" not in content and "def _get_schema" not in content:
                    self.report_issue(
                        "HIGH",
                        f"Tool {tool_file.name} lacks explicit get_schema method. May fail JSON serialization.",
                        tool_file.name,
                    )
                ...
                if "-> dict" not in content and "-> Dict" not in content:
                    self.report_issue(
                        "MEDIUM",
                        f"Tool {tool_file.name} missing type hints on schema/execute.",
                        tool_file.name,
                    )
    ...
    async def audit(self) -> Dict:
        ...
        try:
            ...
            if "WriteFileTool" not in content:
                self.report_issue(
                    "CRITICAL", "CoderAgent missing WriteFileTool import", "agents/coder/agent.py"
                )
            ...
            if "DO NOT wrap tool calls in markdown" not in agent.SYSTEM_PROMPT:
                self.report_issue(
                    "CRITICAL",
                    "CoderAgent System Prompt missing text-based tool forbidden instruction",
            ...
            if not re.search(r"write_file\s*\(\s*['\"]", content):  # Escaped regex roughly
                self.report_issue(
                    "HIGH",
                    "CoderAgent Regex seems to be the old fragile version",
                    "agents/coder/agent.py",
                )
            ...
            if "import textwrap" not in content:
                self.report_issue(
                    "MEDIUM", "CoderAgent missing textwrap for dedenting", "agents/coder/agent.py"
                )

        except ImportError:
            self.report_issue(
                "CRITICAL",
                "CoderAgent module not found. Check path or file existence.",
                "agents/coder/agent.py",
            )
        except Exception as e:
            self.report_issue(
                "CRITICAL",
                f"CoderAgent Instantiation Failed or Audit Error: {e}",
                "agents/coder/agent.py",
            )
...
class OrchestratorAudit(AuditAgent):
    async def audit(self):
        ...
        if "[:50]" in content or "[:100]" in content:
            self.report_issue(
                "CRITICAL",
                "Orchestrator truncates user request ([:50] or similar) before passing to sub-agents. Context is destroyed.",
        ...
        if "router" not in content.lower():
            self.report_issue(
                "HIGH",
                "Orchestrator seems hardcoded, no dynamic router usage detected.",
                "agents/orchestrator/agent.py",
            )
                    ...
                    for i, line in enumerate(lines):
                        if "except Exception as e:" in line or "except Exception:" in line:
                            ...
                            if i + 1 < len(lines) and (
                                ...
                                self.report_issue(
                                    "HIGH",
                                    f"Silent swallow detected in {py_file.name}",
                                    f"Line {i+1}",
                                )
...
class EnvironmentAudit(AuditAgent):
    async def audit(self):
        ...
        if missing:
            self.report_issue(
                "HIGH", f"Missing environment variables: {', '.join(missing)}", ".env"
            )
...
class TestAudit(AuditAgent):
    async def audit(self):
        ...
        if skipped_count > 10:
            self.report_issue(
                "MEDIUM",
                f"High number of skipped tests detected ({skipped_count}). Mocks might be hiding failures.",
                "tests/",
            )
...
class FileSystemAudit(AuditAgent):
    async def audit(self):
        ...
        if not os.access(gemini_dir, os.W_OK):
            self.report_issue(
                "CRITICAL",
                f"Cannot write to {gemini_dir}. Agents cannot persist memory.",
                str(gemini_dir),
            )
...
class LogAudit(AuditAgent):
    async def audit(self):
        ...
        if found_errs > 0:
            self.report_issue(
                "MEDIUM",
                f"Found {found_errs} log files containing existing Error traces. Review required.",
                "logs/",
            )
...
class AgencyStructureAuditAgent(AuditAgent):
    async def audit(self) -> Dict:
        ...
        if not agency_file.exists():
            self.report_issue(
                "HIGH", "core/agency.py missing. Central nervous system absent.", "core/agency.py"
            )
            return
        ...
        try:
            ...
            if "explorer" not in agency._agents:
                self.report_issue(
                    "CRITICAL",
                    "Agency missing 'explorer' alias (User Requirement)",
                    "core/agency.py",
                )
            else:
            ...
            self.report_issue(
                "CRITICAL",
                "Agency module not found. Check path or file existence.",
                "core/agency.py",
            )
        except Exception as e:
            self.report_issue(
                "HIGH", f"Agency Instantiation Failed or Audit Error: {e}", "core/agency.py"
            )
