...
class BenchmarkEvaluator:
    ...
    async def evaluate(
        ...
        try:
            ...
            verification.verification_score = (
                ...
                if verification.benchmark_total > 0 else 0.0
            )
    ...
    def get_evaluation_stats(self) -> Dict[str, Any]:
        ...
        improved = sum(
            1 for r in self._evaluation_history
            if r.status == EvolutionStatus.IMPROVED
        )
        rejected = sum(
            1 for r in self._evaluation_history
            if r.status == EvolutionStatus.REJECTED
        )
        verified = sum(
            1 for r in self._evaluation_history
        ...
        avg_time = (
            sum(r.evaluation_time_ms for r in self._evaluation_history) /
            len(self._evaluation_history)
        )
        avg_kappa = (
            sum(r.kappa_contribution for r in self._evaluation_history) /
            len(self._evaluation_history)
        )
