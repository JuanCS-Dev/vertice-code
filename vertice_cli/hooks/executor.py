...
class HookExecutor:
    ...
    async def execute_hooks(
        self, event: HookEvent, context: HookContext, hooks: List[str]
    ) -> List[HookResult]:
        ...
        for hook_command in hooks:
            ...
            if result.success:
                logger.debug(
                    f"Hook succeeded: {result.command} " f"({result.execution_time_ms:.0f}ms)"
                )
            else:
    ...
    async def execute_hook(
        self, event: HookEvent, context: HookContext, command_template: str
    ) -> HookResult:
    ...
    def _substitute_variables(self, command_template: str, context: HookContext) -> str:
    ...
    async def _execute_direct(self, command: str, cwd: Path) -> HookResult:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                *args, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, cwd=str(cwd)
            )
            ...
            return HookResult(
                success=False, command=command, error=str(e), executed_in_sandbox=False
            )

    async def _execute_sandboxed(self, command: str, cwd: Path) -> HookResult:
        ...
        try:
            sandbox_result = self.sandbox_executor.execute_sandboxed(
                command=command, cwd=cwd, timeout=self.timeout_seconds, readonly=False
            )
