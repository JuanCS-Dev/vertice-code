    ...
    def rename_symbol(self, file_path: Path, old: str, new: str) -> RefactoringResult:
        try:
            ...
            return RefactoringResult(
                True, f"Renamed {count} occurrences", [file_path], f"{old}→{new}: {count}"
            )
    ...
    def organize_imports(self, file_path: Path) -> RefactoringResult:
        try:
            ...
            if not content.strip():
                return RefactoringResult(
                    False, "Empty file", [], "", "Cannot organize imports in empty file"
                )
            ...
            new_content = "\n".join(
                [ast.unparse(i) for i in sorted_imp]
                + ([""] if imports and other else [])
                + [ast.unparse(o) for o in other]
            )
            file_path.write_text(new_content)
            return RefactoringResult(
                True, f"Organized {len(imports)} imports", [file_path], f"Sorted {len(imports)}"
            )
        except Exception as e:
            return RefactoringResult(False, "Failed", [], "", str(e))
