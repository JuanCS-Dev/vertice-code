...
class PreviewMixin:
    ...
    def _show_preview(
        self, path: FilePath, old_content: str, new_content: str, operation: str = "write"
    ) -> bool:
        """
        ...
        self.console.print()
        self.console.print(
            Panel(f"[bold cyan]{operation.upper()}:[/bold cyan] {path}", style="cyan")
        )
        ...
        # Ask confirmation
        self.console.print()
        confirmed = Confirm.ask(f"[bold]Apply changes to {path}?[/bold]", default=True)
...
class PreviewableWriteTool(PreviewMixin):
    ...
    def execute(self, path: str, content: str, preview: bool = True) -> Dict[str, Any]:
        """
        ...
        if not self._show_preview(path, old_content, content, "write"):
            return {"success": False, "message": "Operation cancelled by user", "path": path}
