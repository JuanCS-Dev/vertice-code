...
from typing import Dict, List, Optional, Type, Pattern, ClassVar, Any
...
class BlockRendererRegistry:
    ...
    def get_all_patterns(cls) -> Dict[BlockType, Pattern]:
        ...
        return {r.block_type: r.get_pattern() for r in cls._renderers.values() if r.get_pattern()}

    @classmethod
    def list_renderers(cls) -> List[str]:
        """Lista todos os renderers registrados."""
        return [f"{r.block_type.name} (priority={r.priority})" for r in cls._detection_order]
...
class CodeFenceRenderer(BlockRenderer):
    ...
    def extract_metadata(cls, line: str) -> Dict[str, Any]:
        match = re.match(cls.pattern, line.strip())
        if match:
            return {"fence_marker": match.group(1)[:3], "language": match.group(2) or "text"}
        return {}
    ...
    def render(self, block: BlockInfo) -> RenderableType:
        ...
        return Panel(
            ...
            + ("" if block.is_complete else " [dim]⏳[/dim]"),
