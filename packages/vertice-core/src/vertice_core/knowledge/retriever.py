...
class GraphRetriever:
    ...
    def _multi_hop_retrieve(self, query: RetrievalQuery) -> List[DocumentChunk]:
        ...
        for chunk in all_chunks:
            chunk.relevance_score = 0.6 * chunk.relevance_score + 0.4 * chunk.graph_score
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        avg_time = sum(r.retrieval_time_ms for r in self._retrieval_history) / len(
            self._retrieval_history
        )
        avg_chunks = sum(len(r.chunks) for r in self._retrieval_history) / len(
            self._retrieval_history
        )
        avg_score = sum(r.retrieval_score for r in self._retrieval_history) / len(
            self._retrieval_history
        )
