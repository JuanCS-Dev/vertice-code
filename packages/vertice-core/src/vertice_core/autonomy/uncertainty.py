...
class UncertaintyEstimator:
    ...
    def _decompose_uncertainty(self, estimate: UncertaintyEstimate) -> None:
        ...
        if estimate.mutual_information > 0:
            ...
            estimate.prediction_uncertainty = max(
                0.0, estimate.entropy - estimate.mutual_information
            )
        else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        avg_uncertainty = sum(e.total_uncertainty for e in self._estimation_history) / len(
            self._estimation_history
        )
        avg_epistemic = sum(e.parameter_uncertainty for e in self._estimation_history) / len(
            self._estimation_history
        )
        avg_aleatoric = sum(e.prediction_uncertainty for e in self._estimation_history) / len(
            self._estimation_history
        )
