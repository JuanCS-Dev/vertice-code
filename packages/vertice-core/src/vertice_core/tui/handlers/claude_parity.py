...
class ClaudeParityHandler:
    ...
    def tasks_handler(self):
        ...
        if self._tasks_handler is None:
            from vertice_core.tui.handlers.claude_parity_tasks import ClaudeParityTasksHandler
    ...
    def plan_handler(self):
        ...
        if self._plan_handler is None:
            from vertice_core.tui.handlers.claude_parity_plan import ClaudeParityPlanHandler
    ...
    async def _handle_compact(self, args: str, view: "ResponseView") -> None:
        ...
        """
        from vertice_core.tui.core.context import get_sliding_window, WindowStrategy
    ...
    def _update_token_dashboard(self, compressor) -> None:
        ...
        try:
            from vertice_core.tui.widgets import TokenDashboard
    ...
    async def _handle_context(self, args: str, view: "ResponseView") -> None:
        ...
        """
        from vertice_core.tui.core.context import (
            get_sliding_window,
            get_thought_manager,
        )
    ...
    async def _handle_model(self, args: str, view: "ResponseView") -> None:
        if args:
                ...
                if (
                    ...
                    # If not a provider, try as Gemini model name
                        ...
                        view.add_error(
                            ...
                            f"**Gemini models:** gemini-3-pro, gemini-3-pro, etc."
                        )
                        return
            ...
            if hasattr(self.bridge.llm, "_vertice_coreent") and self.bridge.llm._vertice_coreent:
                ...
                current_provider = self.bridge.llm._vertice_coreent.current_provider or "auto"
