"""
Resilience Patterns Module - Fault Tolerance
============================================

Re-exports from vertice_core.resilience with TUI-specific additions.

Usage:
    # Preferred: direct from core
    from vertice_core.resilience import CircuitBreaker, CircuitState

    # This module (re-exports + TUI additions)
    from vertice_core.tui.core.resilience_patterns import CircuitBreaker, CircuitBreakerStats

Contains:
- CircuitBreaker: Prevents cascading failures (from vertice_core.resilience)
- CircuitBreakerStats: Extended stats for TUI observability
"""

from __future__ import annotations

# Re-export from canonical location
from vertice_core.resilience import (
    CircuitBreaker,
    CircuitBreakerConfig,
    CircuitOpenError as CircuitBreakerOpen,  # Alias for backward compat
    CircuitState,
)

# TUI-specific addition
from vertice_core.tui.core.resilience_patterns.circuit_breaker import (
    CircuitBreakerStats,
)

__all__ = [
    "CircuitBreaker",
    "CircuitBreakerConfig",
    "CircuitBreakerOpen",
    "CircuitBreakerStats",
    "CircuitState",
]
