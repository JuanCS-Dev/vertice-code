...
- CircuitBreaker: vertice_core.tui/core/resilience/circuit_breaker.py
- ToolCallParser: vertice_core.tui/core/parsing/tool_call_parser.py
- Streaming: vertice_core.tui/core/streaming/gemini_stream.py
...
from vertice_core.tui.core.parsing.tool_call_parser import (
    ToolCallParser,
    KNOWN_TOOLS,
)
from vertice_core.tui.core.schema_adapter import SchemaAdapter
from vertice_core.tui.core.streaming.gemini import (
    GeminiStreamConfig,
    GeminiStreamer,
)
...
class GeminiClient:
    ...
    def __init__(
        ...
        self.model_name = model or os.getenv("GEMINI_MODEL") or "gemini-3-flash"
    ...
    def get_provider_status(self) -> str:
        ...
        """
        if self._vertice_coreent:
            status = self._vertice_coreent.get_provider_status()
    ...
    def get_current_provider_name(self) -> str:
        ...
        if self._vertice_coreent:
            available = self._vertice_coreent.get_available_providers()
            current = self._vertice_coreent.current_provider
            if current:
                return current
            if available:
                return available[0]
    ...
    def set_provider(self, name: str) -> bool:
        ...
        """
        if self._vertice_coreent:
            success = self._vertice_coreent.set_preferred_provider(name)
            if success:
    ...
    async def stream(
        ...
        **kwargs: Any,
        ...
        if self._vertice_coreent:
            ...
            # Collect chunks and check for exhaustion error
            exhausted = False
            async for chunk in self._stream_via_client(
                prompt, system_prompt, context, tools, **kwargs
            ):
                if "All providers exhausted" in chunk:
                    exhausted = True
                    logger.warning("VerticeClient exhausted, falling back to direct Gemini API")
                    break
                yield chunk
            ...
            logger.info("Using direct Gemini API as fallback")
    ...
    async def _stream_open_responses_via_client(
        ...
        if hasattr(self._vertice_coreent, "stream_open_responses"):
            async for sse_event in self._vertice_coreent.stream_open_responses(
    ...
    def is_available(self) -> bool:
        ...
        if self._vertice_coreent:
            return bool(self._vertice_coreent.get_available_providers())
