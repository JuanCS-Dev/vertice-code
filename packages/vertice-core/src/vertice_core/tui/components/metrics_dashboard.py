...
class MetricsDashboard:
    ...
    def _render_full(self) -> Panel:
        ...
        table = Table(show_header=False, box=SIMPLE, padding=(0, 2), expand=False)
        ...
        # Tokens
        tokens_k = self.metrics.tokens_used / 1000
        table.add_row("Tokens Used", Text(f"{tokens_k:.1f}K", style=f"bold {COLORS['neon_cyan']}"))
        ...
        if self.metrics.error_count > 0:
            ...
            table.add_row(
                "Errors",
                Text(
                    f"{self.metrics.error_count} ({error_rate:.1f}%)",
