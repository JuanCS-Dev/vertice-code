...
class ContextSuggestionEngine:
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        imports.append({"name": alias.name, "type": "import", "line": node.lineno})
                ...
                # Definitions
                    ...
                    definitions.append({"name": node.name, "type": "function", "line": node.lineno})

                elif isinstance(node, ast.ClassDef):
                    definitions.append({"name": node.name, "type": "class", "line": node.lineno})
    ...
    def suggest_related_files(
        self, file_path: Path, max_suggestions: int = 10
    ...
    def _is_internal_import(self, import_name: str) -> bool:
        ...
        project_modules = {"vertice_core", "tests"}
